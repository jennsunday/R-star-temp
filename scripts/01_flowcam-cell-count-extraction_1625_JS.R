...
#### remove mixed up vials, TT162 and BB162 (mixed on Aug 25th 2016) and remove contaminated vials CH162 and CH255####
Rstar_Augexpt <- Rstar_Augexpt %>%
  unite(unique_ID, species, temperature, replicate, remove = FALSE) %>%
  filter(unique_ID != "BB_16_2") %>%
  filter(unique_ID != "TT_16_2") %>%
  filter(unique_ID != "CH_16_2") %>%
  filter(unique_ID != "CH_25_5")
