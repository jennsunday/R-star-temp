...
#combine actual monod fits to predict rstar (black points)
...
all_rstar %>%
  ...
  theme_bw() 
#+ geom_point(data=rstar_estimate, aes(y=rstar_calc, x=Temperature), inherit.aes=FALSE, alpha=0.05)
...
all_rstar %>%
  ...
  coord_cartesian(ylim = c(0, 5.5)) + geom_line(alpha=0.5)+
  theme_bw() 
#+ geom_point(data=rstar_estimate, aes(y=rstar_calc_ks_fixed, x=Temperature), inherit.aes=FALSE, alpha=0.05)
