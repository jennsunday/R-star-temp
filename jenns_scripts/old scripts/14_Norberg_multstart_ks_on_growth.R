...
for(k in 1:4){
  ...
  fit<-nls_multstart(log.Particles.per.ml 
                     ~ N_init + (a*exp(b*Temperature)*(1-((Temperature-z)/(w/2))^2)) * 
                       (N.Treatment / (ks_int + (ks_slope*Temperature) + N.Treatment)) * day,
                     data= data,  iter = 250,
                     start_lower = c(N_init=4, a = 0.1, b=0.0001, z=5, w=5, ks_int = -5, ks_slope=0),
                     start_upper = c(N_init=7, a = 0.6, b=0.2, z=40, w=40, ks_int = 5, ks_slope=1),
                     supp_errors = 'Y',
                     convergence_count = 100,
                     na.action = na.omit,
                     lower=c(N_init=0, a = 0, b=0, z=-20, w=0, ks_int = -20, ks_slope=-20))
  ...
  z<-filter(params_fit, term=="z")$estimate
  w<-filter(params_fit, term=="w")$estimate
  ks_int<-filter(params_fit, term=="ks_int")$estimate
  ks_slope<-filter(params_fit, term=="ks_slope")$estimate
  
  #at what temp does ks cross zero?
  tempcross<-uniroot(function(T){ks_int+ks_slope*T},interval=c(-5,100))$root
  
  #get root for every temperature
  roots_NB<-data.frame() #make an empty dataframe - temporary
  roots_NB_below<-data.frame() #make an empty dataframe - temporary
  for(i in filter(newdata, Temperature>tempcross)$Temperature){ #for temperatures where ks is positive
    NB_fix<-function(N){
      growth_rate<-a*exp(b*i)*(1-((i-z)/(w/2))^2) * 
        N/(ks_int + (ks_slope*i)  + N) - 0.1}
    thisroot<-try(uniroot(NB_fix, interval=c(0,200)), TRUE) #try taking the root
    roots_NB<-rbind(roots_NB, data.frame(root=ifelse(class(thisroot)!="try-error", thisroot$root, 200),
                                         Temperature=i)) #add this to the output, setting root at a large number (200) if not within interval
  }
  
  for(i in filter(newdata, Temperature<tempcross)$Temperature){ #for temperatures where ks is negative, set to 0
    NB_fix<-function(N){
      growth_rate<-a*exp(b*i)*(1-((i-z)/(w/2))^2) * N/(0.1 + N) - 0.1}
    thisroot<-try(uniroot(NB_fix, interval=c(0,200)), TRUE) #try taking the root
    roots_NB_below<-rbind(roots_NB_below, data.frame(root=ifelse(class(thisroot)!="try-error", thisroot$root, 200),
                                         Temperature=i)) #add this to the output, setting root at a large number (200) if not within interval
  }
  roots_ks_umax<-mutate(rbind(roots_NB_below, roots_NB), umax=a*exp(b*newdata$Temperature)*(1-((newdata$Temperature-z)/(w/2))^2), 
                        ks=ks_int+ks_slope*newdata$Temperature, Species=unique(alldata$Species)[k])
  all_roots_ks_umax<-rbind(all_roots_ks_umax, roots_ks_umax)
}
...
for(k in 1:4){
  ...
  fit_boots <- data %>%     
    modelr::bootstrap(n = 3, id = 'boot_num') %>%           #bootstrap data n times
    group_by(boot_num) %>%
    mutate(fit = map(strap, ~nls_multstart(log.Particles.per.ml 
                                           ~N_init + (a*exp(b*Temperature)*(1-((Temperature-z)/(w/2))^2)) * 
                                             (N.Treatment / (ks_int + (ks_slope*Temperature) + N.Treatment)) * day,
                                           data= data.frame(.),  iter = 250,
                                           start_lower = c(N_init=4, a = 0.1, b=0.0001, z=5, w=5, ks_int = 0, ks_slope=0),
                                           start_upper = c(N_init=7, a = 0.6, b=0.2, z=40, w=40, ks_int = 5, ks_slope=5),
                                           supp_errors = 'Y',
                                           convergence_count = 100,
                                           na.action = na.omit,
                                           lower=c(N_init=0, a = 0, b=0, z=-20, w=0, ks_int = -20, ks_slope=-20))
  ...
  for (j in unique(params_boot$boot_num)){ #for each boot
    ...
    z<-filter(params_boot, boot_num==j &  term=="z")$estimate
    w<-filter(params_boot, boot_num==j & term=="w")$estimate
    ks_int<-filter(params_boot, boot_num==j &  term=="ks_int")$estimate
    ks_slope<-filter(params_boot, boot_num==j &  term=="ks_slope")$estimate
    ...
    roots_ks_umax_temp<-mutate(rbind(roots_NB_below, roots_NB), boot_num=j, 
                               umax=a*exp(b*newdata$Temperature)*(1-((newdata$Temperature-z)/(w/2))^2), 
                               ks=ks_int+ks_slope*newdata$Temperature, Species=unique(alldata$Species)[k]) #roots, ks, and umax from one boot
    roots_ks_umax_boot<-rbind(roots_ks_umax_boot, roots_ks_umax_temp) #store all the boots together
  }
...
all_roots_ks_umax %>%
  ...
  theme_bw() + ylab("umax") + facet_grid(~Species) +
  geom_line(aes(x=Temperature, y=umax, colour=as.factor(Species))) + 
  geom_ribbon(data=all_boots_confint, aes(x=Temperature, ymin = umax_lwr_CI, ymax = umax_upr_CI), alpha = .2)
...
all_roots_ks_umax %>%
  ...
  geom_ribbon(data=all_boots_confint, aes(x=Temperature, ymin = ks_lwr_CI, ymax = ks_upr_CI), alpha = .2)
...
all_roots_ks_umax %>%
  ...
  theme_bw() + ylab("umax") + facet_grid(~Species) +
  geom_line(aes(x=Temperature, y=root, colour=as.factor(Species))) +
  geom_ribbon(data=all_boots_confint, aes(x=Temperature, ymin = root_lwr_CI, ymax = root_upr_CI), alpha = .2)
