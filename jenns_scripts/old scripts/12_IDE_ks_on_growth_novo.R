...
all_summary_fit_fix<-data.frame()
...
for(k in 1:4){
  ...
  all_summary_fit_IDE_kg_fix<-rbind(all_summary_fit_fix, summary_fit)
...
#View(all_params_fit)
  ...
  for(i in 1:length(newdata$Temperature)){
    ...
    NB_fix<-function(N){
      growth_rate<-(((b1 * exp(b2*Temperature))
                     -(d0 + d1*exp(d2*Temperature))) * (N / (ks + N))) - 0.1}
  ...
  roots_ks_umax<-mutate(roots_NB, umax=(b1 * exp(b2*Temperature))
...
####################################
all_params_fit<-data.frame()
all_summary_fit_var<-data.frame()
all_preds_fit<-data.frame()
all_roots_ks_umax<-data.frame()
...
for(k in 1:4){
  ...
  summary_fit<-glance(fit) %>% mutate(Species=unique(alldata$Species)[k])
  all_summary_fit_var<-rbind(all_summary_fit_var, summary_fit)
