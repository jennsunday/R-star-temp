...
#so reduce the "searching" scope of starting conditions so that the time to fit each model is shorter
#but might need to increase this scope
...
fits_each_N_TT <-data.frame(curve.id.list, topt.list,maxgrowth.list,z.list,w.list,a.list,b.list,rsqr.list,s.list,n.list)

#add species and run columns - do this carefully and check
df   <- dat.full[1:700,3:4]
fits_each_N_TT_full   <- cbind(df, fits_each_N_TT)
write_csv(fits_each_N_TT_full, "data-processed/Norberg_fits_by_N_TT.csv")
head(dat.full)
...
fits_each_N_TT_no_zero <- fits_each_N_TT_full %>%
  filter(maxgrowth.list>0.01)

par(mfrow=c(3,3))
for(i in c(1,3,5,7,2,4,6)){
  temppoints<-dat.full %>%
    filter(N.Treatment==unique(dat.full$N.Treatment)[i])
  ...
  tempcurves<-fits_each_N_TT_no_zero %>%
    filter(N.Treatment==unique(dat.full$N.Treatment)[i]) 
  for(j in 1:100){
...
for (i in c(1,3,5,7,2,4,6)){
  nitratemodel<-fits_each_N_TT_no_zero %>%
    filter(N.Treatment==unique(N.Treatment)[i])
  for(j in nitratemodel$run){
    r_predict<-with(subset(nitratemodel, nitratemodel$run==j), nbcurve(newdata,z.list,w.list,a.list,b.list))
    dftemp<-data.frame(r_predict=r_predict, temperature=newdata, run=j, N.Treatment=nitratemodel$N.Treatment[1])
...
write.csv(r_pred_all, "data-processed/Norberg_fits_by_N_TT_predictions.csv")
