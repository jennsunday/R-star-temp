...
sp3<-subset(cdata, cdata$species==3)
sp4<-subset(cdata, cdata$species==4)
sp1c<-subset(sp1, sp1$temp<20 | sp1$date<"2017-05-18")
sp1c<-subset(sp1, sp1$temp<30 | sp1$date<"2017-05-15")
sp2c<-subset(sp2, sp2$date<"2017-06-06")
sp3c<-subset(sp3, sp3$date<"2017-06-06")
cdata<-rbind(sp1c, sp2c, sp3c, sp4)
...
#plot by temp and species along day and fit michaelis mentin model
par(mfcol=c(6,4))
par(mar=c(3,3,0,0), oma=c(1,1,0.5, 0.5))
results<-data.frame(species=c(rep(1,6),rep(2,6),rep(3,6),rep(4,6)), temp=c(rep(c(3, 10, 17, 24, 31, 38), 4)), nstar=rep(99, 24))
for(j in 1:4){
for (i in c(3, 10, 17, 24, 31, 38)){
  tempdata<-subset(cdata, cdata$temp==17 & cdata$species==3)
  with(tempdata, plot(n.fixed~day, ylim=c(0, 12), col=j))

  #model
  a_start = 4
  k_start = 0.01
  curve <- nls(n.fixed~5-(a*(day)/(k+day)), data=tempdata, start=list(a=a_start,k=k_start), 
               algorithm = "port")
  
  new.data <- data.frame(day = seq(min(tempdata$day),max(tempdata$day),len = 100))
  lines(new.data$day,predict(curve,newdata = new.data))
  results$nstar[results$species==j & results$temp==i]<-predict(curve, new.data)[100]
  results$nstar_SE[results$species==j & results$temp==i]<-summary(curve)$parameters[1,2]
}
}
?nls
predict(curve, new.data)[100]

#plot Rstar as a function of temp
par(mfrow=c(1,1))
with(results, plot(nstar~temp))
for(i in 1:4){
  with(subset(results, results$species==i), lines(nstar~temp, col=i))
  with(subset(results, results$species==i), segments(temp, nstar-nstar_SE, temp, nstar+nstar_SE, col=i))
}



#plot by temp and species along day and fit decay model and equilibrium mean model
par(mfcol=c(6,4))
par(mar=c(3,3,0,0), oma=c(1,1,0.5, 0.5))
results<-data.frame(species=c(rep(1,6),rep(2,6),rep(3,6),rep(4,6)), temp=c(rep(c(3, 10, 17, 24, 31, 38), 4)), nstar=rep(99, 24))
for(j in 1:4){
  for (i in c(3, 10, 17, 24, 31, 38)){
    ...
    a_start = 6
    b_start = -0.001
    curve <- nls(n.fixed~a*exp(-b*day), data=tempdata, start=list(a=a_start,b=b_start))
...
par(mfrow=c(1,1))
tempdata<-subset(cdata, cdata$temp==24 & cdata$species==1)
with(tempdata, plot(meanabs~day, ylim=c(0, 0.12), col=17)) 

a_start = 0.05
b_start = -0.001
curve <- nls(meanabs~a*exp(-b*day), data=tempdata, start=list(a=a_start,b=b_start))

new.data <- data.frame(day = seq(min(tempdata$day),max(tempdata$day),len = 100))
lines(new.data$day,predict(curve,newdata = new.data))
predict.nls(curve,newdata = new.data, se.fit=TRUE)
summary(curve)$parameters[1,2]
...
##################################
#convert absorbance data to nitrate
#################################
names(data)
plot(data$date.measured)
plot(data$date.measured)
plot(data$date.sampled)
#for data before July 2nd, use standard curve from 280617; 
#for data from after July 2nd, use standard curve from 110717 
#(or, consider interpolating standard curve for every day between standard curve creation)
coef(st.2806)[1]
