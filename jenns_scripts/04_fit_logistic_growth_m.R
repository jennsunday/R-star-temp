...
cell_results %>%
  mutate(corrected_r=r+0.1) %>% #correct for an added death rate of 0.1 per day
  ggplot(aes(x = temp, y = corrected_r, color = factor(species))) + geom_point(size = 2) +
  theme_bw() + geom_line() + 
