...
for(k in 1:4){
  ...
  fit<-nls_multstart(log.Particles.per.ml 
                        ~ N_init + (((a*exp(b*Temperature)*(1-((Temperature-z)/(w/2))^2)) * 
                            ((N.Treatment + tn)/ (ks + N.Treatment + tn))) * day),
                        data= data,  iter = 250,
                        ...
                        na.action = na.omit,
                        lower=c(N_init=0, a = 0, b=0, z=-20, w=0, ks = 0, tn=-100))
...
for(k in 1:4){
  ...
  fit<-nls_multstart(log.Particles.per.ml 
                     ~ N_init + (a*exp(b*Temperature)*(1-((Temperature-z)/(w/2))^2)) * 
                       ((N.Treatment + tn)/ ((ks_a^(Temperature+ks_b)) + N.Treatment + tn)) * day,
                     data= data,  iter = 250,
                     start_lower = c(N_init=4, a = 0.1, b=0.0001, z=5, w=5, ks_a = 0, ks_b=0, tn=-5),
                     start_upper = c(N_init=7, a = 0.6, b=0.2, z=40, w=40, ks_a = 5, ks_b=5, tn=0),
                     supp_errors = 'Y',
                     convergence_count = 100,
                     na.action = na.omit,
                     lower=c(N_init=0, a = 0, b=0, z=-20, w=0, ks_a =0, ks_b=-20, tn=-100))
...
for(k in 1:4){
  ...
  fit_boots <- data %>%     
    ...
    mutate(fit = map(strap, ~nls_multstart(log.Particles.per.ml 
                                           ...
                                           convergence_count = 100,
                                           na.action = na.omit,
                                           lower=c(N_init=0, a = 0, b=0, z=-50, w=0, ks_a =0, ks_b=-50, tn=-100),
                                           upper=c(N_init=200, a = 200, b=200, z=200, w=200, ks_a =200, ks_b=5, tn=50))
                     ))
