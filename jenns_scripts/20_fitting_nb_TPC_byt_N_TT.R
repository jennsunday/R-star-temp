...
fits_each_N_CH <-data.frame(curve.id.list, topt.list,maxgrowth.list,z.list,w.list,a.list,b.list,rsqr.list,s.list,n.list)

#add species and run columns - do this carefully and check
#df   <- dat.full[1:700,3:4]
#fits_each_N_CS_full   <- cbind(df, fits_each_N_CS)
write_csv(fits_each_N_CH, "data-processed/Norberg_fits_by_N_CH.csv")
head(dat.full)
...
fits_each_N_CH_full_no_zero <- fits_each_N_CH %>%
...
for(i in c(1,3,5,7,2,4,6)){
  ...
  tempcurves<-fits_each_N_CH_full_no_zero %>%
...
for (i in c(1,3,5,7,2,4,6)){
  nitratemodel<-fits_each_N_CH_full_no_zero %>%
...
write.csv(r_pred_all, "data-processed/Norberg_fits_by_N_CH_predictions.csv")
