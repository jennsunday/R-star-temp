...
for(i in 1:100){
  ...
  lines(sort(newdata$Temperature), pred$fit[order(newdata$Temperature)], col="#41ab5d")
  ...
  name<-data.frame(ks=pred$fit, Temperature=newdata$Temperature, run=i, Species="TT")
