...
plot(dat.full$growth.rate[dat.full$curve.id == 2]~dat.full$temperature[dat.full$curve.id == 2],ylim=c(pmin(-1.5,min(dat$growth.rate)),max(dat$growth.rate)+(0.2)*max(dat$growth.rate)),main=curve.id.list[2],
		 xlim=c(min(dat$temperature)-2,max(dat$temperature)+2),xlab='Temperature',ylab='Specific growth rate (per day)', cex.lab=1.5,cex.axis=1.5)
curve(nbcurve(x,fits$z.list[2],fits$w.list[2],fits$a.list[2],fits$b.list[2]),col='red', lwd=2,add=TRUE)

plot(dat.full$growth.rate[dat.full$curve.id == 3]~dat.full$temperature[dat.full$curve.id == 3],ylim=c(pmin(-1.5,min(dat$growth.rate)),max(dat$growth.rate)+(0.2)*max(dat$growth.rate)),main=curve.id.list[3],
		 xlim=c(min(dat$temperature)-2,max(dat$temperature)+2),xlab='Temperature',ylab='Specific growth rate (per day)', cex.lab=1.5,cex.axis=1.5)
curve(nbcurve(x,fits$z.list[3],fits$w.list[3],fits$a.list[3],fits$b.list[3]),col='red', lwd=2,add=TRUE)


plot(dat.full$growth.rate[dat.full$curve.id == 4]~dat.full$temperature[dat.full$curve.id == 4],ylim=c(pmin(0,min(dat$growth.rate)),max(dat$growth.rate)+(0.2)*max(dat$growth.rate)),main=curve.id.list[4],
		 xlim=c(min(dat$temperature)-2,max(dat$temperature)+2),xlab='Temperature',ylab='Specific growth rate (per day)', cex.lab=1.5,cex.axis=1.5)
